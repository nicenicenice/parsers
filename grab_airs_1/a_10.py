...
def getDataFromDocumentROV(el, isArrival=False):
    scheduled = ""
    flightno = ""
    status = ""
    actual = ""
    ...
    try:
        ...
        if rawStatus.find(" ") > 0:
            rawStatus = rawStatus[:rawStatus.find(" ")]
...
# ARRIVE
urlROVArrive = "http://www.rnd-airport.ru/1linetablo.ajax.5.19.php?arrive"
...
for el in g.doc.select("//online_arrive/flight"):
    arrival = getDataFromDocumentROV(el, True)
    if arrival != -1:
...
urlROVDepart = "http://www.rnd-airport.ru/1linetablo.ajax.5.19.php?depart"
...
resp = g.go(urlROVDepart, post=todayDepartedParams)

for el in g.doc.select("//online_depart/flight"):
    departure = getDataFromDocumentROV(el)
    if departure != -1:
...
resultROV = {}
resultROV["airport_id"] = "ROV"
resultROV["departures"] = departures
resultROV["arrivals"] = arrivals
jsonResult = json.dumps(resultROV)
#print jsonResult